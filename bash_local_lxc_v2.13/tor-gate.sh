...
lxc launch ubuntu-minimal:jammy $_container_name

# Добавляем запись в /etc/hosts внутри контейнера
lxc exec $_container_name -- bash -c "echo '127.0.0.1 $_container_name' >> /etc/hosts"
...
# Разрешение UDP-пакетов на порту 5353
echo "Разрешение UDP-пакетов на порту 5353..."
lxc exec $_container_name -- bash -c "iptables -A INPUT -p udp --dport 5353 -j ACCEPT"

# Блокировка остальных UDP-пакетов
#echo "Блокировка остальных UDP-пакетов..."
...
lxc exec $_container_name -- bash -c "sysctl -p"
