...
OVPN_CONFIG="$(pwd)/$UNIQUE_NUMBER/*.ovpn"
OVPN_AUTH="$(pwd)/$UNIQUE_NUMBER/*.txt"
...
if ! ls $OVPN_CONFIG > /dev/null 2>&1; then
   echo "Файл конфигурации OpenVPN в директории $UNIQUE_NUMBER не найден."
   exit 1
fi

if ! ls $OVPN_AUTH > /dev/null 2>&1; then
   echo "Файл аутентификации OpenVPN в директории $UNIQUE_NUMBER не найден."
   exit 1
fi
