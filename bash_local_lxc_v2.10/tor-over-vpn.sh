...
# Запрашиваем IP-адрес шлюза
echo "Введите IP-адрес для шлюза $_container_name:"
read ip_address_gate

# Выводим информацию о параметрах
echo "Настраиваем контейнер $_container_name с IP-адресом $_ip_address и шлюзом $ip_address_gate."
...
lxc exec $_container_name -- bash -c "echo -e '[Match]\nName=eth1\n\n[Network]\nAddress=$_ip_address/24\nGateway=$ip_address_gate\nDNS=1.1.1.1\nDNS=10.0.4.200\nMTU=1418' > /etc/systemd/network/10-eth1.network"

# Настройка DNS
lxc exec $_container_name -- bash -c "echo -e 'DNS=$_DNS' >> /run/systemd/network/eth1.network"
...
lxc exec $_container_name -- apt install -y iptables iptables-persistent curl nano inetutils-ping tor
...
# Блокировка входящих UDP-пакетов
lxc exec $_container_name -- bash -c "iptables -A INPUT -p udp -j DROP"

# Блокировка исходящих UDP-пакетов
lxc exec $_container_name -- bash -c "iptables -A OUTPUT -p udp -j DROP"
...
# Отключаем роутинг
lxc exec $_container_name -- bash -c "echo 'net.ipv4.ip_forward = 0' | tee -a /etc/sysctl.conf"
lxc exec $_container_name -- sysctl -p
